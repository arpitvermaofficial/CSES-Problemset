...
void solve()
{
    /*
    Approach 
    we solve this question by consdier two pair sum equal to givern sum
    fist pair  is generate by given current i and every no behind that i
    and second pair is generated in forward direction such that current i with j
    which moves forward direction
    when we find x- second pair sum in map this means there is pair exist behind i
    whose sum is considered as first sum and the resultannt sum is x
    */
     ll n ,x;
    cin >> n >> x;
    vector<ll>v(n);
