...
void solve()
{
    /*
    Approach
    We have to check if the next no in increasing array is occured before in original array
    it means we have to one more round to collect this no otherwise this no is collectes in same round as its
    postion is after the no in original array
    */
    ll n;
    cin >> n;
    vector<ll>v(n);
