...
void solve()
{
    ...
    by brute force we have to use three loop which exceed time complexity
    and for second inner loop third inner loop run approx n times
    but by two pointer approach on second loop and third loop 
    it only iterates n which reduce complexity n2 to n
    */
    ll n ,x;
    cin >> n >> x;
    ...
    for( int i=0; i<n; i++ ) {
        ...
        for(ll j=i+1, k=n-1; j<k; j++) {
            while(j<k && v[j].first+v[k].first>x2) {
                k--;
            }
