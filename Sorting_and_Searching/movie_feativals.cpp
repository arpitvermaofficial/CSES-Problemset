...
void solve()
{
    /*
    Approach
    As we have to increase the movie count so we focus on those movie
    which ends as early possible i.e small duration movie
    so sort movie list by ending time then inc
    count if any movie time is greater or equal to end time
    of last movie
