...
{
    /*
    Approach we have to make tower as small cube is above large cube
    so we use upper bound to as if element is upper bound end of array such that is maximum among all
    so we make that cube as another tower and if upper bound exist within array means 
    there is something block which has larger size so place small block above it and replace it
    */
    ll n;
    cin >> n;
    vector<ll>v;
