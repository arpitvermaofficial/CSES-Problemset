...
void solve()
{
    /*
    Approach 
    This is standard problem of finding next greater or smaller element using motonic stack
    intitution for using monotonic stack is we have to find next smaller no and in monotonic stack
    every element contains the immediate smaller element than its self 
    so for searching it reduces complexity as any element we search from top
    if its not small we search for next element for which it is large basicall the bottom element 
    and it continues till we found small elelement  
    */
    ll n;
    cin >> n;
    vector<ll>v(n);
