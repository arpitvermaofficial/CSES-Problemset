...
void solve()
{
     /*
    Approach
    In this we store prfix sum as if the add no to current sum and if x- current sum 
    finds in map that means we to remove that subarray whose sum is x-current sum
    */
   ll n, x;
   cin >> n >> x;
   vector<ll>v(n);
